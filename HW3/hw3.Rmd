...
icustays_tble %>%
  ...
  left_join(labevents_tble, by = "subject_id","hadm_id") %>%
  left_join(chartevents_tble, by = "subject_id","hadm_id") %>%
  ungroup() %>%
  show_query() %>%
  as_tibble() %>%
  mutate(deathin30 = ifelse(is.na(deathtime) == TRUE, "No",
                      ifelse(deathtime - admittime <= 43200, "Yes", "No"))) %>%
  print(width = Inf) %>%
  saveRDS(str_c("./mimiciv_shiny/icu_cohort.rds"))

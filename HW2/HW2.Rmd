...
knitr::include_graphics("Agreement.png")
```
...
```{r eval=FALSE}
admissionread_csv<- read_csv(str_c(mimic_path, "/core/admissions.csv.gz"))
```
```{r eval=FALSE}
admissionread.csv<- read.csv(str_c(mimic_path, "/core/admissions.csv.gz"))
```
```{r eval=FALSE}
admissionfread<- fread(str_c(mimic_path, "/core/admissions.csv.gz"))
```
...
```{r}
filter(admission,hospital_expire_flag=1)%>%
